...
Если команда `pip` не распознается в PowerShell, используйте только формат через интерпретатор:
...
## Сборка Windows EXE + Inno Setup инсталлятора

В проект добавлены:

- `build_installer.bat` - автоматическая сборка
...
1. создаст `venv` (если его нет),
2. установит зависимости,
3. соберет `dist\VirtualCom.exe` через PyInstaller,
4. соберет инсталлятор через `ISCC`.

Итоговый установщик будет в папке `output`.